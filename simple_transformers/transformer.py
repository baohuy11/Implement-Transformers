...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, enc_output, self_mask, enc_mask):
        ...
        cross_attn_output = self.cross_attn(x, enc_output, enc_output, enc_mask)
...
# Training loop (using random data for demonstration)
transformer.train()
...
transformer.eval()
with torch.no_grad():
    src_sample = torch.randint(1, src_vocab_size, (1, max_seq_length))
    generated_seq = torch.tensor([[1]])
    max_gen_length = 20

    for _ in range(max_gen_length):
        output = transformer(src_sample, generated_seq)
    ...
    print("Generated Sequence (Token IDs):", generated_seq)