...
from mha_model import build_transformer
from config import get_weigths_file_path, get_config, latest_weights_file_path
...
def get_or_build_tokenizer(config, ds, lang):
    # config['tokenizer_path'] = '../tokenizers/tokenizer_{0}.json'
...
def get_ds(config):
    ...
    # Keep 90 % for training and 10 % for valiation
...
def train_model(config):
    ...
    model_filename = latest_weights_file_path(config) if preload == 'latest' else get_weigths_file_path(config, preload) if preload else None
    if model_filename:
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        # Save the model ath the end of the epoch
